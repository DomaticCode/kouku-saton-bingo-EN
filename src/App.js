...
function App() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      for (let j = 0; j < 5; j++) {
        ...
        tds.push(
          <td key={`block${i}${j}`}>
            <div
              className={`bingo-block ${cls}`}
              onClick={() => {
                clickBingo(i, j);
              }}
            >
              {inner}
              {stillListening ? <div className="bingo-coord">{`${i + 1} ${j + 1}`}</div> : null}
            </div>
          </td>
        );
      }
      res.push(<tr key={`block${i}`}>{tds}</tr>);
    }
    return (
  ...
  const transNum = (x) => {
    console.log(x);
    x = `${x}`.replace(/\s+/g, "");
    if (["one", "하나", "1", 1, "열"].includes(x)) return 1;
    if (["two", "둘", "2", 2, "스물"].includes(x)) return 2;
    if (["three", "셋", "3", 3, "서른"].includes(x)) return 3;
    if (["for", "four", "4", 4, "마흔"].includes(x)) return 4;
    if (["five", "or", "5", 5, "쉰"].includes(x)) return 5;
    return 0;
  };
  ...
  const commands = [
    ...
    ////// if STT reads "bum" instead of bomb
    {
      command: "Bum :i:j",
      callback: (i, j) => clickBingo(transNum(i) - 1, transNum(j) - 1),
    },
    ...
    ////// if STT reads "bom" instead of bomb
    {
      command: "Bom :i:j",
      callback: (i, j) => clickBingo(transNum(i) - 1, transNum(j) - 1),
    },
    ...
    ////// if STT reads "bump" instead of bomb
    {
      command: "Bump :i:j",
      callback: (i, j) => clickBingo(transNum(i) - 1, transNum(j) - 1),
    },
    ...
    {
      command: "Bomb for five",
      callback: (i, j) => clickBingo(4 - 1, 5 - 1),
    },
    {
      command: "Bomb four five",
      callback: (i, j) => clickBingo(4 - 1, 5 - 1),
    },
    {
      command: "Bomb for five.",
    ...
    {
      command: "Bom for five",
      callback: (i, j) => clickBingo(4 - 1, 5 - 1),
    },
    {
      command: "Bom four five",
      callback: (i, j) => clickBingo(4 - 1, 5 - 1),
    },
    {
      command: "Bom for five.",
    ...
    {
      command: "Bum for five",
      callback: (i, j) => clickBingo(4 - 1, 5 - 1),
    },
    {
      command: "Bum four five",
      callback: (i, j) => clickBingo(4 - 1, 5 - 1),
    },
    {
      command: "Bum for five.",
    ...
    const script = [
      [
        ...
        `처음에 놓인 두 해골 위치를 클릭해서 입력해주세요. 이후 폭탄을 놓을 때마다 그 위치를 클릭해주면 알아서 십자로 적용해줍니다.\n
         잘못 클릭했을 경우 [취소] 버튼으로 되돌릴 수 있고, 처음부터 다시 할 경우 [reset] 버튼을 누르면 됩니다.`,
      ],
      [
      ...
      [`Q. 저는 더블모니터가 아닌데요?`, `그런 당신을 위해 (아마도) 모바일에서도 보기 편하도록 디자인했습니다. 님폰없?`],
      [
        `Q. 패턴 피하고 딜하느라 바쁜데요?`,
        `그런 당신을 위해 음성인식 모드를 넣었습니다. 음성인식 스위치를 켜면 음성 인식을 시작합니다. 단, 초록색으로 표시되는 인식한 단어를 보면 느끼시겠지만 인식률이 높지는 않아서 천천히 또박또박 말해주셔야 합니다.\n
         ...
         취소: [취소]버튼과 동일한 효과입니다.\n
         reset: [reset]버튼과 동일한 효과입니다.\n
         이난나: 이난나 모드를 on/off 합니다.\n
         음성 해제: 음성인식 모드를 해제합니다.\n
         All credit goes to ialy1595 for development of this tool, visit them on their website/github at the top of the page or show them some support through the link below.`,
      ],
    ];
      ...
      <div className="desc-container">
        {script.map((x, i) => (
          <div key={`desc-${i}`}>
            ...
            {x[1].split("\n").map((y, j) => (
              <div className="desc-answer" key={`desca=${i}${j}`}>
                {y}
              </div>
            ))}
          </div>
      ...
      <Container maxWidth="sm">
        <div className="tool-container">
          <div className="head-container">
            <div className="message-warning">{`※ 처음 오셨다면 아래 문답을 먼저 읽어주세요!`}</div>
          ...
          <div className="button-container">
            <div className="button-wrap">
              <Button variant="contained" onClick={resetBingo}>
                reset
              </Button>
            </div>
            <div className="button-wrap">
              <Button variant="contained" className="btn" onClick={cancleBingo}>
                cancel
              </Button>
            </div>
          ...
          <div className="slider-container">
            <div className="slider-desc">{`해골 위 폭탄`}</div>
        ...
        <div className="dona">
          <a href="https://donaricano.com/mypage/1709725536_JvgCKV" target="_blank" rel="noreferrer">
            <img src="https://d1u4yishnma8v5.cloudfront.net/donarincano_gift.png" alt="donaricano-btn" className="dona-img" />
          </a>
        </div>
      </Container>
    </div>
